...
/**
 * 仪表盘面板：webview 中文 UI + 历史 ring buffer（300 点）重放。
 * 消息协议：
 *   ext → webview: {type:'update', state} | {type:'history', history}
 *   webview → ext: {type:'ready'} | {type:'setThreshold', warn, critical}
 */
export class DashboardPanel implements vscode.Disposable {
  ...
  push(state: RenderState): void {
    ...
    if (this.panel) {
      void this.panel.webview.postMessage({ type: 'update', state: this.snapshot(state) });
    }
  }
  ...
  show(): void {
    ...
    this.panel.webview.html = this.buildHtml(mediaDir);
  ...
  private buildHtml(mediaDir: string): string {
    const uri = (f: string) => this.panel!.webview.asWebviewUri(vscode.Uri.file(path.join(mediaDir, f))).toString();
...
<body>
  ...
  private onMessage(msg: { type: string; warn?: number; critical?: number; sessionId?: string; title?: string; path?: string }): void {
    switch (msg.type) {
      case 'ready': {
        // webview 就绪 → 回放历史 + 阈值配置 + 余额开关状态 + 当前状态
        if (this.panel) {
          void this.panel.webview.postMessage({ type: 'config', ...this.getThresholds(), balanceEnabled: this.getBalanceEnabled() });
          void this.panel.webview.postMessage({ type: 'history', history: this.history });
          if (this.lastState) void this.panel.webview.postMessage({ type: 'update', state: this.lastState });
        }
        break;
      }
      case 'setThreshold': {
      ...
      case 'setTitle': {
        if (typeof msg.sessionId === 'string' && msg.sessionId) {
          this.onSetTitle(msg.sessionId, typeof msg.title === 'string' ? msg.title : '');
        }
        break;
      }
      case 'pinSession': {
